...
\begin{figure}[h]
\begin{center}
\includegraphics[width=10cm]{pictures/equation_kalman.png}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=10cm]{pictures/equation_ekf.png}
...
If there is no new measurement until the time of the a priori estimated state, the correction step is not executed. This allows the algorithm to run the EKF with a higher rate, then the IMU and GPS provide data. To summarize: If we have no new data from the sensors the system just keeps propagating based on the physical model. If there is at least one new measurement from any sensor block 4 will be executed.
